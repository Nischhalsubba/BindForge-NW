    ...
    <article className={`bind-card ${props.selected ? "is-selected" : ""} ${copyState === "copied" || copyState === "fallback" ? "is-copied" : ""}`} data-gsap-enter data-preset-id={props.preset.id}>
      <header className="card-header">
        ...
        <div className="card-header-actions">
          <button aria-label={`${props.favourite ? "Remove" : "Add"} ${props.preset.title} ${props.favourite ? "from" : "to"} favourites`} aria-pressed={props.favourite} className="icon-text-button favourite-button" onClick={props.onFavourite} type="button"><Icon name="star" /></button>
